...
export const onRequest: PagesFunction = async (context) => {
  ...
  try {
    ...
    if (!identifier || !code || !newPassword) {
      return new Response(JSON.stringify({ error: 'identifier, code e newPassword são obrigatórios' }), { status: 400, headers: corsHeaders });
    }
    const strong = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$/.test(newPassword);
    if (!strong) {
      return new Response(JSON.stringify({ error: 'Senha fraca. Use no mínimo 8 caracteres, com letras maiúsculas, minúsculas e números.' }), { status: 400, headers: corsHeaders });
    }
    ...
    // ...continuação da lógica de reset...
