...
## Backend (Turso on Vercel)

- Prerequisite: set `DATABASE_URL` and `DATABASE_AUTH_TOKEN` in Vercel Project Settings → Environment Variables with your Turso connection info.
...
- Locally (with Vercel), run `vercel dev` and hit `http://localhost:3000/api/setup` after ensuring `DATABASE_URL` and `DATABASE_AUTH_TOKEN` are set.
...
Ensure `DATABASE_URL` and `DATABASE_AUTH_TOKEN` are set in an `.env` file or your shell environment for local dev.
